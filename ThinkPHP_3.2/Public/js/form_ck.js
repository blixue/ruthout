//XMLHttpRequest   
    ...
    function Ajax(data){
        xmlhttp.open("POST","/ThinkPHP_3.2/index.php/Home/User/register?username="+document.getElementById("username").value,true);
        xmlhttp.send(null);
...
function checkEmail(email) {
    if (chekemail(email.value) == false) {
        ...
        change_submit("true");
            ...
            ajax.open('post','/ThinkPHP_3.2/index.php/Home/User/register?email='+email.value,true)
            ajax.send(null)
...
function chkMobile(mobile){
    ...
    if(pattern.test(mobile)) {
        ...
        showInfo("mobile_notice", msg_can_rg);
        change_submit("true");
    }
    else {
        mobile.className = "FrameDivWarn";
        showInfo("mobile_notice", mobile_phone_invalid);
        change_submit("false");
    }
}
